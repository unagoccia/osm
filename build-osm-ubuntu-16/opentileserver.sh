...
AUTH_TEXT='/root/auth.txt'

#User for DB and rednerd
OSM_USER='osm';			#system user for renderd and db
...
#apt-get -y install python zlib1g-dev clang make pkg-config
#source bootstrap.sh
...
if [ "${WEB_MODE}" == 'ssl' ]; then
	...
	if [ ! -f /etc/apache2/ssl/server.key -o ! -f /etc/apache2/ssl/server.crt ]; then
		...
		if [ $(grep -m 1 -c 'ssl pass' "${AUTH_TEXT}") -eq 0 ]; then
			echo "ssl pass: ${SSL_PASS}" >> "${AUTH_TEXT}"
		fi
		cd /etc/apache2/ssl/
